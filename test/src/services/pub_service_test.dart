...
void main() {
  group(PubService, () {
    ...
    group('getCurrentVersion when called', () {
      testWithOverrides('with global installation should return version',
          () async {
        when(() => processService.runPubGlobalList()).thenAnswer(
          (i) async => ['$ksDolphinCli $version'],
        );
    ...
    group('hasLatestVersion when called', () {
      ...
      testWithOverrides('with current version is up to date should return true',
          () async {
        when(() => processService.runPubGlobalList()).thenAnswer(
          (i) async => ['$ksDolphinCli $version'],
        );
    ...
    testWithOverrides('update when called should call pub updater update',
        () async {
      when(() => pubUpdater.update(packageName: any(named: 'packageName')))
          .thenAnswer(
        (i) async => ProcessResult(pid, exitCode, stdout, stderr),
